...
def obtem_fator_J_AGMA_20graus(N_pinhao, N_coroa):
    ...
    """
    if N_pinhao <= 17: return 0.24
    elif N_pinhao <= 19: return 0.25
    elif N_pinhao <= 21: return 0.26
    elif N_pinhao <= 25: return 0.27
    elif N_pinhao <= 30: return 0.28
    else: return 0.29
...
def calcula_Km_AGMA(b_face, d_p, montagem_tipo='comercial', precisao_engrenagem='Q6'):
    ...
    if montagem_tipo == 'precisao': C_pm = 1.0
    elif montagem_tipo == 'comercial': C_pm = 1.1
    else: C_pm = 1.2

    if montagem_tipo == 'precisao': A, B, C = 0.0675, 0.0128, -0.0000765
    elif montagem_tipo == 'comercial': A, B, C = 0.127, 0.0158, -0.0000765
    else: A, B, C = 0.247, 0.0167, -0.0000765

    C_ma = A + (B * b_face_pol) + (C * b_face_pol**2)

    if montagem_tipo == 'precisao': C_e = 0.8
    elif montagem_tipo == 'comercial': C_e = 1.0
    else: C_e = 1.0
...
def calcula_Kv_AGMA(V_t, Qv, phi_rad):
    ...
    if V_t_ftmin == 0: K_v = 1.0
    else: K_v = ((A_coef + np.sqrt(V_t_ftmin)) / A_coef) ** B_coef
...
def calcula_grupo_engrenagens_auto(i_alvo, m, N_p, W_t_estimado, S_at, K_m_estimado=1.5, K_v_estimado=1.2, J_estimado=0.24):
    phi_graus = 20.0
    phi_rad = np.radians(phi_graus)
    ...
    b_face = calcula_largura_face_automatica(
        m=m, d_p=d_p, W_t=W_t_estimado, S_at=S_at,
        K_m=K_m_estimado, K_v=K_v_estimado, J=J_real
    )
    ...
    Z = (np.sqrt(r_ext_p**2 - r_b_p**2) + np.sqrt(r_ext_c**2 - r_b_c**2) - C * np.sin(phi_rad))
    mp = Z / p_base
...
def calcula_vida_util_engrenagens(grupoengre, forcas, sigma_b, sigma_c, S_at, S_ac, vida_util_desejada_horas=4000):
    ...
    if sigma_b > S_at:
        ratio = S_at / sigma_b
        if ratio < 0.1: N_flexao = 100
        else: N_flexao = N_base * (ratio ** (1/abs(b_flexao)))
    else:
        N_flexao = 10**10
    ...
    if sigma_c > S_ac:
        ratio = S_ac / sigma_c
        if ratio < 0.1: N_pitting = 100
        else: N_pitting = N_base * (ratio ** (1/abs(c_pitting)))
    else:
        N_pitting = 10**10
    ...
    if vida_min >= vida_util_desejada_horas: status = "APROVADO"
    elif vida_min >= vida_util_desejada_horas * 0.7: status = "MARGINAL"
    else: status = "REPROVADO"

    # --- CORREÇÃO AQUI: Retornando as chaves que o utilities.py exige ---
    return {
        'vida_flexao_horas': vida_flexao_horas * fator_total,   # Adicionado de volta
        'vida_pitting_horas': vida_pitting_horas * fator_total, # Adicionado de volta
...
def analisa_fatores_com_vida(grupoengre, forcas, S_at, S_ac, C_p, vida_util_horas=4000, montagem_tipo='comercial', precisao_engrenagem='Q6'):
    ...
    FS_flexao = S_at / sigma_b if sigma_b > 0 else 999
    ...
    FS_pitting = (S_ac / sigma_c) ** 2 if sigma_c > 0 else 999
    ...
    if FS_flexao >= 2.0 and FS_pitting >= 1.2 and vida_util['status_vida'] != "REPROVADO":
...
def dimensionamento_engrenagens_com_vida(T_in, n_in, i_estagio, eta_estagio, S_at, S_ac, C_p, vida_util_horas=4000):
    ...
    for Np in N_pinhao_padronizado:
        for m in modulos_padronizados:
            ...
            if dados['erro_estagio'] > 0.05: continue
    ...
    aprovadas2 = [p for p in guardaengrenagem2 if p['status_calc'] == "Aprovado"]
    marginais2 = [p for p in guardaengrenagem2 if p['status_calc'] == "Marginal"]
